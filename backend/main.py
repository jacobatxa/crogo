...
async def create_project(
    ...
    proj_id = db.create_project(name, sponsor, tid, str(dest), file.filename)
...
def generate_project(proj_id: int):
    ...
    if not mappings:
        raise HTTPException(400, "请先在模板库完成占位符映射")
    ...
    out_name = f"project_{proj_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
    ...
    try:
        ...
        return GenerateResponse(
            success=True,
            message="文档生成成功",
            download_url=f"/api/projects/{proj_id}/download",
        )
    except Exception as e:
...
def download_project(proj_id: int):
    row = db.get_project(proj_id)
