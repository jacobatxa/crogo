...
const API = {
  ...
  previewProject(file) {
    const fd = new FormData();
    fd.append('file', file);
    return this.request('/api/projects/preview', { method: 'POST', body: fd });
  },
  ...
  getProjectReview(id) {
    return this.request(`/api/projects/${id}/review`);
  },

  saveFields(id, fields, opts = {}) {
    return this.request(`/api/projects/${id}/fields`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ fields, confirmed: !!opts.confirmed }),
    });
  },
